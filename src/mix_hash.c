...
mix_hash_t* mix_hash_init(int per_block,int size) {
    ...
    hash->free_node_num = per_block;
    hash->free_node_head = malloc(sizeof(hash_list_node_t));
    hash->free_node_head->next = NULL;
    for(int i = 0; i < per_block; i++){
        hash_list_node_t* node = malloc(sizeof(hash_list_node_t));
        node->key = -1;
        node->value = -1;
        node->next = hash->free_node_head->next;
        hash->free_node_head->next = node;
    }
    hash->free_node_lock = malloc(sizeof(pthread_spinlock_t));
    pthread_spin_init(hash->free_node_lock,0);
 ...
 */
static inline hash_list_node_t* mix_hash_node_get(mix_hash_t* hash, uint32_t key,
                                                  uint32_t value) {
    hash_list_node_t* node = hash->free_node_head->next;
    if(unlikely(node == NULL)) abort();
    hash->free_node_head->next = node->next;
    node->key = key;
    node->value = value;
    node->next = NULL;
    hash->free_node_num--;
 ...
 */
static inline void mix_hash_node_free(mix_hash_t* hash,hash_list_node_t* node) {
    node->key = -1;
    node->value = -1;
    pthread_spin_lock(hash->free_node_lock);
    node->next = hash->free_node_head->next;
    hash->free_node_head->next = node;
    hash->free_node_num++;
    pthread_spin_unlock(hash->free_node_lock);
}
...
void mix_hash_put(mix_hash_t* hash, uint32_t key, uint32_t value) {
    ...
    hash_list_node_t* new_node = mix_hash_node_get(hash,key, value);
...
mix_kv_t mix_hash_get_entry_by_idx(mix_hash_t* hash,int idx) {
    ...
    if (hash_node->len > 0){
        ...
        mix_hash_node_free(hash,hash_node->list);
